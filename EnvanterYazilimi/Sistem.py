...
def build_ui(new_window):
    ...
    def temizle():
        ...
        entry_varlik_kodu.insert(0, veritabani.siradaki_kodu_getir("sistem", "SS"));
        entry_varlik_kodu.configure(state="readonly")
        entry_sistem_adi.delete(0, tk.END)
        combo_departman.set('');
        combo_sorumlu.set('')
        kayitlari_getir()
    ...
    def kayitlari_getir(aranan_metin=""):
        ...
        sorgu = "SELECT * FROM sistem WHERE 1=1";
        parametreler = []
        if aranan_metin != "":
            sorgu += " AND (varlik_grubu LIKE ? OR varlik_kodu LIKE ? OR sistem_adi LIKE ? OR departman LIKE ? OR sorumlu LIKE ?)"
            parametreler.extend(['%' + aranan_metin + '%'] * 5)
    ...
    def ekle():
        gerekli_alanlar = [combo_varlik_grubu.get().strip(), entry_sistem_adi.get().strip(),
                           combo_departman.get().strip(), combo_sorumlu.get().strip()]
        ...
        baglanti.cursor().execute(
            "INSERT INTO sistem (varlik_grubu, varlik_kodu, sistem_adi, departman, sorumlu) VALUES (?, ?, ?, ?, ?)",
            (combo_varlik_grubu.get(), entry_varlik_kodu.get(), entry_sistem_adi.get(), combo_departman.get(),
             combo_sorumlu.get()))
        baglanti.commit();
        baglanti.close();
        messagebox.showinfo("Başarılı", "Sistem altyapısı kaydedildi.");
        temizle()
    ...
    def tablo_tiklandi(event):
        ...
        if degerler:
            ...
            entry_varlik_kodu.configure(state="readonly")
            entry_sistem_adi.delete(0, tk.END);
            entry_sistem_adi.insert(0, degerler[3])
            combo_departman.set(degerler[4]);
            combo_sorumlu.set(degerler[5])

    def sil():
        if not tablo.focus(): return
        if messagebox.askyesno("Onay", "Altyapı silinsin mi?"):
            baglanti = sqlite3.connect("envanter.db");
            baglanti.cursor().execute("DELETE FROM sistem WHERE varlik_kodu=?",
    ...
    def guncelle():
        ...
        baglanti.cursor().execute("INSERT OR IGNORE INTO departmanlar (ad) VALUES (?)",
                                  (combo_departman.get().strip(),))
        baglanti.cursor().execute(
            "UPDATE sistem SET varlik_grubu=?, sistem_adi=?, departman=?, sorumlu=? WHERE varlik_kodu=?",
            (combo_varlik_grubu.get(), entry_sistem_adi.get(), combo_departman.get(), combo_sorumlu.get(), degerler[2]))
        baglanti.commit();
        baglanti.close();
    ...
    ctk.CTkLabel(new_window, text="Sistem", font=("Segoe UI", 24, "bold"), text_color="#1E293B").pack(
        anchor="w", pady=(0, 5))
    ...
    ctk.CTkLabel(inner_form, text="Sistem Adı:", font=lbl_font, text_color=lbl_color, width=110, anchor="w").grid(row=1,
                                                                                                                  column=0,
                                                                                                                  padx=8,
                                                                                                                  pady=4)
    entry_sistem_adi = ctk.CTkEntry(inner_form, width=280, height=30, font=ent_font, fg_color=ent_bg,
                                    border_color=ent_border, border_width=1, text_color=ent_color)
    entry_sistem_adi.grid(row=1, column=1, padx=8, pady=4)
    ...
    combo_departman.grid(row=1, column=3, padx=8, pady=4, ipady=2)
    combo_departman.configure(postcommand=lambda: combo_departman.configure(values=veritabani.departmanlari_getir()))

    ctk.CTkLabel(inner_form, text="Sorumlu:", font=lbl_font, text_color=lbl_color, width=110, anchor="w").grid(row=2,
                                                                                                               column=0,
                                                                                                               padx=8,
                                                                                                               pady=4)
    combo_sorumlu = ttk.Combobox(inner_form, width=28, state="readonly", font=ent_font)
    combo_sorumlu.grid(row=2, column=1, padx=8, pady=4, ipady=2)
    ...
    sutunlar = ("ID", "Varlık Grubu", "Varlık Kodu", "Sistem Adı", "Departman", "Sorumlu")
    genislikler = {"ID": 40, "Varlık Grubu": 180, "Varlık Kodu": 80, "Sistem Adı": 200, "Departman": 140,
                   "Sorumlu": 140}
